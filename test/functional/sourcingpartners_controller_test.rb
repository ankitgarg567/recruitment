...
class SourcingpartnersControllerTest < ActionController::TestCase
  ...
  test "should destroy sourcingpartner" do
   ...
   test "create a new sourcingpartner" do
    post :create, :sourcingpartner => {:name => "XYZ Consultancy", :description => "Head office is located in new delhi."}
    sourcingpartner = assigns(:sourcingpartner)
    assert_not_nil sourcingpartner
    assert sourcingpartner.errors.count == 0
    assert_equal "Sourcing partner successfully created.", flash[:notice]
  end
end
